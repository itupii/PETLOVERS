...
export default class ListagemConsumoMais extends Listagem{
    ...
    public async listar(): Promise<void> {
        ...
        consumoProduto.sort(function(n1: { quantidadeProdutos: number; },n2: { quantidadeProdutos: number; }) {
            return n2.quantidadeProdutos - n1.quantidadeProdutos
        })
        consumoProduto = consumoProduto.slice(0,10)

        consumoServico.sort(function(n1: { quantidadeServicos: number; },n2: { quantidadeServicos: number; }) {
        ...
        while (exec) {
            ...
            switch (opcao) {
                case 1:
                    ...
                    consumoProduto.forEach( (cliente: { nome: string; cpf: string; quantidadeProdutos: string; }) => {
                ...
                case 2:
                    ...
                    console.log(chalk.red(`\n---------------------------------------------------------------\n`));
                    consumoServico.forEach( (cliente: { nome: string; cpf: string; quantidadeServicos: string; }) => {
                        console.log(chalk.blueBright('Nome do Cliente: ') + cliente.nome  );
                        console.log(chalk.blueBright('CPF do Cliente: ') + cliente.cpf  );
